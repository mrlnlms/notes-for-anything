...
describe('CompanionRegistry', () => {
  ...
  describe('múltiplos companions pro mesmo binário (caso patológico)', () => {
    it('regra é um companion por binário — last writer wins, perdedor vira nota normal', async () => {
      ...
      // Um deles é o ativo, o outro virou nota normal (sem registro de binário).
      const active = registry.getCompanionFor('paper.pdf');
      expect(active).not.toBeNull();
      expect(['paper.pdf.md', 'outro/paper.pdf.md']).toContain(active);

      const loser = active === 'paper.pdf.md' ? 'outro/paper.pdf.md' : 'paper.pdf.md';
      expect(registry.getBinaryFor(active!)).toBe('paper.pdf');
      expect(registry.getBinaryFor(loser)).toBeNull();
    });
  });
